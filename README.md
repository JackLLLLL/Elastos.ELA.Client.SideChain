...
## Summary
##### ELA.Client is a command line tool for the full node of ELA side chain. There are three parts of commands in it. One of the parts includes some common wallet functions, for example create wallet, transfer elacoin, add account, show balance etc. Another part supplies query interface of transaction and block. The Third part let you can do some management things of ELA full node, like change debug level, change mining mode etc.
##### This repo is used to hold source code of ELA side chain command tool. There is another repo, Elastos.ELA.Client, holds the source code of main chain command tool. They are both depends on the repo named Elastos.ELA.Client.Core.
